...
from FuN.scripts.training.feudal_networks.models.models import (linear, conv2d, build_lstm, normalized_columns_initializer)
import FuN.scripts.training.feudal_networks.policies.policy_utils as policy_utils
from FuN.scripts.training.feudal_networks.policies.configs.lstm_config import config
...
class LSTMPolicy(object):
    ...
    def __init__(self, obs_space, act_space,global_step):
        ...
        vfhid = tf.nn.elu(linear(x, config.size, "value_hidden", normalized_columns_initializer(0.01)))
        self.vf = tf.reshape(linear(vfhid, 1, "value", normalized_columns_initializer(1.0)), [-1])

        # retrieve logits, sampling op
        self.logits = linear(x, act_space, "action", normalized_columns_initializer(0.01))
